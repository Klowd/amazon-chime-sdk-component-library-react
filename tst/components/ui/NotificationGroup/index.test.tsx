...
const getNotificationGroupTree = () => {
  ...
  return (
    <NotificationProvider>
      <NotificationGroup />
      <StorybookTestButton label='Add simple INFO notification' payload={payloadForInfoNotification} />
      <StorybookTestButton label='Add simple ERROR notification' payload={payloadForErrorNotification} />
      <StorybookTestButton label='Add autoclosing INFO notification' payload={payloadForAutoclosingInfoNotification} />
    </NotificationProvider>
  );
}
...
describe('NotificationGroup', () => {
  ...
  describe('With NotificationProvider', () => {
    ...
    test('NotificationGroup should show a notification on a button click', () => {
      ...
      const addInfoNotificationButtonEl = getByText('Add simple INFO notification');
    ...
    test('NotificationGroup should show a info notification output message when message is provided', () => {
      ...
      let notifications = getAllByTestId('notification');
      const infoNotificationEl = notifications[0];
      expect(infoNotificationEl).toContainElement(getByText('Info notification'));
    });
    ...
    test("NotificationGroup should remove a notification if clicked on that notification's close button", () => {
      ...
      const { getByText, getAllByTestId } = renderWithTheme(lightTheme, tree);
      const addInfoNotificationButtonEl = getByText('Add simple INFO notification');
      const addErrorNotificationButtonEl = getByText('Add simple ERROR notification');
      fireEvent.click(addInfoNotificationButtonEl);
    ...
    test('NotificationGroup should remove notification when notification autoClose is passed', async () => {
      ...
      const addAutoclosingInfoNotificationButtonEl = getByText('Add autoclosing INFO notification');
      ...
      expect(autoClosingNotificationEl).toContainElement(getByText('Autoclosing INFO notification'));
